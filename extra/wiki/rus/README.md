...
известное устройство [USB rubber ducky](https://shop.hak5.org/products/usb-rubber-ducky-deluxe), сделанное командой Hak5, при этом имеет широкие возможности, меньшую цену  
и полностью открытые исходные файлы. Оно выглядит как самый обычный USB диск, но при  
этом работает как клавиатура, которая исполняет заранее записанную серию команд. Таким  
образом можно решать широкий спектр IT задач от настройки сети до установки удалённого  
доступа, так как устройство фактически может делать всё то же самое что и человек в  
...
читает команды напрямую в текстовом формате, что убирает необходимость устанавливать дополнительное  
ПО, например для преобразования payload.txt в inject.bin  
...
проект разработан с использованием KiCad 5.1.5  
...
на диске. Для избежания порчи файловой системы рекоммендуется не извлекать  
...
и отлючит функции камуфляжа, независимо от того что написано в config.txt  
...
Программатор используемый для прошивки устройства - ST-Link V2  
...
Она была успешно скомпилирована и протестирована с arm-none-eabi-gcc версии 8.3.1  
...
#### /firmware/ --------------- содержит makefile, скрипт компоновщика, исходный код; это каталог для компиляции  
...
Цена за 1 устройство 2000 рублей, возможна доставка почтой России. Стоимость доставки по РФ  
204 рубля при покупке от 1 до 8 едениц. Для доставки нужно будет указать адрес получателя:  
Имя Фамилия,  
Адрес, Город, Регион  
индекс  
номер телефона (необязательно)  
  
После отправки я передам вам трек-номер для отслеживания посылки.  
Ожидаемое время доставки по РФ 1-2 недели, за границу 3-4 недели.  
