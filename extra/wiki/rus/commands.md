Язык команд, используемый в рабочих (payload) файлах, основан на существующем [ducky script](https://github.com/hak5darren/USB-Rubber-Ducky/wiki/Duckyscript),  
...
* "WAITFOR_INIT"  --- ждёт, пока хост-компьютер завершит инициализацию устройства  
* "WAITFOR_CAPSLOCK"  --- ждёт, пока пользователь вручную не переключит нажмёт Capslock 2 раза  
* "WAITFOR_RESET"  --- ждёт, пока хост-компьютер не перезагрузит устройство (например, при перезагрузке самого ПК)  
...
по её значению HID keycode (например, вы хотите нажать какую-то редкую клавишу).  
Допустимые значения HID keycode  — от 0 до 221 (десятичное число).  
...
мыши. Она используется всякий раз, когда вам нужно постоянно нажимать какие-то клавиши,  
например как при открытии меню выбора языка Windows 10 с помощью клавиш GUI + SPACE. Эта  
...
указанное количество раз. Если повторить нужно целый блок команд, то начало этого  
блока должен быть помечено командой "REPEAT_START". Размер таких блоков неограничен,  
поэтому он может охватывать даже весь рабочий скрипт целиком. Как только исполнение  
команды "REPEAT **n**" завершено, предыдущий блок команд считается завешенным. Для нового  
блока опять нужно будет указать начало командой "REPEAT_START". В случае использования  
команды "REPEAT **n**" без явного указания начала блока команд, только 1 команда  
непосредственно перед "REPEAT **n**" будет повторена. Некоторые примеры:  
\
